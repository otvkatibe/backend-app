...
jest.mock('ioredis', () => {
  ...
  return class Redis {
    ...
    async set(key: string, value: string) { storage.set(key, value); return 'OK'; }
    async del(key: string) { storage.delete(key); return 1; }
    quit() { return Promise.resolve('OK'); }
    duplicate() { return this; }
    // Stub for rate-limit-redis call
    async call(command: string, ...args: any[]) {
      // If command is script load, return a fake SHA
      if (command.toLowerCase() === 'script') return 'e0e1f9fabfc9d4800c877a703b823ac0578ff8db';
      return 1;
    }
  };
});
...
jest.mock('rate-limit-redis', () => {
  return class RedisStore {
    constructor() { }
    init() { }
    increment(key: string) { return Promise.resolve({ totalHits: 1, resetTime: new Date() }); }
